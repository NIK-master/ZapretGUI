...
namespace ZapretGUI.Views
{
    public partial class WizardWindow : Window
    {
        ...
        private async void LoadCurrentPage(bool animate = true)
        {
            if (animate)
            {
                _isAnimating = true;
            ...
            for (var i = 0; i < _pages.Count; i++)
            {
                var dot = new Ellipse
                {
                    ...
                    Fill = i == _currentPageIndex
                        ? new SolidColorBrush((System.Windows.Media.Color)System.Windows.Media.ColorConverter.ConvertFromString("#107C10")) // Активная зеленая
                        : new SolidColorBrush((System.Windows.Media.Color)System.Windows.Media.ColorConverter.ConvertFromString("#333333")) // Серая неактивная
                };
        ...
        private async void BtnNextStep_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (_currentPageIndex == _pages.Count - 2)
            {
                ...
                var stepFinish = _pages[3] as Step5_FinishPage;

                var useZapret = stepServices?.UseZapret ?? true;
                var useTgProxy = stepServices?.UseTgProxy ?? true;

                if (stepFinish != null)
                    await stepFinish.RunSetupAsync(true, useZapret, useTgProxy);
